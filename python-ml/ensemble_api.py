...
"""
Ensemble Model API
FastAPI to serve ensemble model predictions to .NET backend
"""
...
async def health_check():
    return HealthResponse(
        ...
        message='Ensemble Model API is running',
...
async def predict(request: PredictionRequest):
    try:
        if request.model_type == 'ensemble' or request.model_type == 'default':
            result = ensemble_api.predict_ensemble(request.features)
        elif request.model_type == 'xgb':
            result = ensemble_api.predict_xgb(request.features)
        else:
            raise HTTPException(status_code=400, detail='Invalid model_type. Use "ensemble" or "xgb"')
